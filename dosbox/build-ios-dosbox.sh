#!/bin/bash
# Build a one-tap "Ultima I in DOSBox" app for your iPhone/iPad and install it,
# with YOUR copy of the game.
#
# This is the *complete* Ultima I (the real DOS game running in DOSBox), unlike
...
# your own Ultima I data (never committed).
#
# Prereqs:
...
#   - Your Ultima I game folder (the one with ULTIMA.EXE). On a Mac GOG
#     install that's usually /Applications/Ultima I™.app/Contents/Resources/game
#
...
U1_SRC="${2:-/Applications/Ultima I™.app/Contents/Resources/game}"
BUNDLE_ID="${U1DOS_BUNDLE_ID:-info.u1redux.u1dos}"
WORK="${HOME}/Library/Caches/u1-dosbox"
DOSPAD="$WORK/dospad"
...
if [ ! -f "$U1_SRC/ULTIMA.EXE" ] && [ ! -f "$U1_SRC/ultima.exe" ]; then
  echo "ERROR: no Ultima I data at:" >&2
  echo "  $U1_SRC" >&2
  echo "Pass the folder with ULTIMA.EXE as the 2nd argument." >&2
  exit 1
fi
...
#     Set U1DOS_KEEP_THUMBNAIL=1 to keep it (e.g. you have a paid account and want the
#     Files thumbnails). This only edits references, and aborts untouched if anything
#     unexpected is found, so it's safe.
if [ -z "${U1DOS_KEEP_THUMBNAIL:-}" ]; then
...
if ! grep -q "Ultima I one-tap" "$EMU"; then
  echo "Patching dospad to auto-run Ultima I ..."
  python3 - "$EMU" <<'PY'
import sys
p = sys.argv[1]
s = open(p).read()
...
inject = marker + '''
    ...
    // Ultima I one-tap boot: if ULTIMA.EXE is present at the C drive root, run it
    // directly (dedicated-app behaviour, independent of package-type detection).
    {
        DPDrive *cDrive = [self.package findDrive:'C'];
        if (cDrive) {
            NSString *u1exe = [cDrive.sourceUrl.path stringByAppendingPathComponent:@"ULTIMA.EXE"];
            if ([[NSFileManager defaultManager] fileExistsAtPath:u1exe]) {
...
# 3b. Ultima I branding: swap the app icon (gold ankh) and rename to "Ultima I".
MASTER="$SCRIPT_DIR/ultima1-icon.png"
ICON_SET="$DOSPAD/Resources/Assets.xcassets/AppIcon.appiconset"
if [ -f "$MASTER" ] && [ -d "$ICON_SET" ]; then
  echo "Applying the Ultima I app icon ..."
...
plutil -replace CFBundleDisplayName -string "Ultima I" "$DOSPAD/Resources/iDOS-Info.plist" 2>/dev/null || true
...
# 3f. Lock the app to landscape. Ultima I is a 4:3 landscape game, and only the landscape
...
\$99 Developer Program. Do this once (takes 2 minutes):
  ...
  5. Then re-run this script to copy your Ultima I data onto the device
...
cp "$U1_SRC"/* "$STAGE/" 2>/dev/null || find "$U1_SRC" -maxdepth 1 -type f -exec cp {} "$STAGE/" \;
rm -f "$STAGE/manual.pdf" "$STAGE"/dosbox*.conf 2>/dev/null || true
printf '{\n  "name": "Ultima I",\n  "autorun": "ULTIMA.EXE"\n}\n' > "$STAGE/idos.json"
echo "Copying your Ultima I data onto the device ..."
...
# 8. Launch — boots straight into Ultima I.
...
echo "  VPN & Device Management, then reopen. It boots straight into Ultima I."
